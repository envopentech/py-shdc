...
from .encryption import SHDCCrypto, secure_compare, zero_memory
...
class KeyManager:
    ...
    def __init__(self, device_id: int, storage_path: Optional[str] = None):
        ...
        if storage_path is None:
            home = Path.home()
            storage_path = home / ".shdc" / "keys" / f"{device_id:08X}"
    ...
    def generate_device_keys(self, force: bool = False) -> tuple[Ed25519PrivateKey, Ed25519PublicKey]:
        """
        ...
        with self._lock:
            ...
            if not force and key_id in self._keys:
                ...
                private_key = SHDCCrypto.deserialize_ed25519_private_key(private_key_bytes)
            ...
            self._store_key(key_id, private_key_bytes, "ed25519_private", 
                          expires_at=time.time() + self.device_key_lifetime)
            
            # Store public key
            public_key_bytes = SHDCCrypto.serialize_ed25519_public_key(public_key)
            self._store_key(f"{key_id}_public", public_key_bytes, "ed25519_public",
                          expires_at=time.time() + self.device_key_lifetime)
    ...
    def generate_session_key(self, peer_device_id: int) -> bytes:
        ...
        with self._lock:
            ...
            # Store key
            self._store_key(key_id, session_key, "aes256",
    ...
    def generate_broadcast_key(self, key_version: int) -> bytes:
        ...
        with self._lock:
            ...
            # Store derived key
            self._store_key(key_id, broadcast_key, "aes256",
    ...
    def store_peer_public_key(self, peer_device_id: int, public_key: bytes):
        ...
        with self._lock:
            ...
            self._store_key(key_id, public_key, "ed25519_public",
                          metadata={"peer_device_id": peer_device_id})
    ...
    def _store_key(self, key_id: str, key_bytes: bytes, key_type: str,
                   expires_at: Optional[float] = None, metadata: Optional[Dict] = None):
