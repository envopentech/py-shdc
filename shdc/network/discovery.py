...
class HubDiscovery:
    ...
    async def discover_hubs(
        ...
        try:
            ...
            await self._send_discovery_request(
                transport, device_private_key, device_public_key, device_info
            )
            ...
            self.logger.info(
                f"Discovery completed: found {len(self.discovered_hubs)} hubs"
            )
    ...
    async def discover_hubs_with_retry(
        self, device_private_key, device_public_key, device_info: str = "SHDC Sensor"
    ) -> List[DiscoveredHub]:
        """
        ...
        while retry_count < self.max_retries:
            self.logger.info(
                f"Hub discovery attempt {retry_count + 1}/{self.max_retries}"
            )

            hubs = await self.discover_hubs(
                device_private_key, device_public_key, device_info
            )
    ...
    async def _send_discovery_request(
        self,
        transport: UDPTransport,
        device_private_key,
        device_public_key,
        ...
        try:
            ...
            # Convert bytes to Ed25519PrivateKey if needed
            if isinstance(device_private_key, bytes):
                private_key_obj = SHDCCrypto.deserialize_ed25519_private_key(
                    device_private_key
                )
            else:
                private_key_obj = device_private_key

            signature = SHDCCrypto.sign_message(private_key_obj, signed_data)
        ...
        try:
            ...
            if hub_id in self.discovered_hubs:
                ...
                if addr[0] != existing_hub.address:
                    self.logger.debug(
                        f"Hub {hub_id:08X} responded from different address: {addr[0]}"
                    )
                return
