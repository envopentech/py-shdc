...
class UDPTransport:
    ...
    def __init__(
        self, port: int = SHDC_PORT, bind_ip: str = "127.0.0.1"
    ...
    async def start(self):
        ...
        try:
            ...
            try:
                mreq = socket.inet_aton(SHDC_MULTICAST_IP) + socket.inet_aton(
                    "0.0.0.0"
                )  # Multicast binding  # nosec B104
    ...
    def get_local_addresses(self) -> List[str]:
        ...
        try:
            ...
            if HAS_NETIFACES:
                for interface in netifaces.interfaces():  # type: ignore
                    addrs = netifaces.ifaddresses(interface)  # type: ignore
                    if netifaces.AF_INET in addrs:  # type: ignore
                        for addr_info in addrs[netifaces.AF_INET]:  # type: ignore
