...
class SHDCHubCLI:
    ...
    async def run_hub(
        ...
        try:
            ...
            # Start protocol (if method exists)
            start_method = getattr(self.protocol, "start", None)
            if start_method:
                await start_method()
            ...
            while self.running:
                try:
                    ...
                    # Log connected devices (if method exists)
                    get_devices_method = getattr(
                        self.protocol, "get_connected_devices", None
                    )
                    if get_devices_method:
                        connected_devices = get_devices_method()
                        if debug and connected_devices:
                            logger.debug(f"Connected devices: {len(connected_devices)}")
                            for device_info in connected_devices:
                                device_hex = f"{device_info.device_id:08X}"
                                logger.debug(
                                    f"  Device {device_hex}: {device_info.device_type}"
                                )
            ...
            if self.protocol:
                ...
                # Note: stop() method not implemented in current SHDCProtocol
    ...
    def reset_keys(self, device_id: int, confirm: bool = False):
        ...
        if not confirm:
            ...
            response = input(
                f"Are you sure you want to reset keys for device {device_hex}? "
                "(yes/no): "
            )
...
Examples:
  ...
  shdc-hub reset-keys 0x12345678             # Reset all keys
    ...
    run_parser.add_argument(
        "device_id", help="Hub device ID (hex format, e.g., 0x12345678)"
    )
    run_parser.add_argument(
        "-i",
        "--interface",
