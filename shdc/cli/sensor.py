...
class SHDCSensorCLI:
    ...
    async def run_sensor(self, device_id: int, device_type: str,
                        auto_join: bool = True, hub_address: Optional[str] = None,
                        data_file: Optional[str] = None, interval: int = 30,
                        debug: bool = False):
        """
        Run SHDC sensor.
        ...
        try:
            ...
            logger.info(f"UDP transport started")
            ...
            # Set device type
            self.protocol.device_type = device_type
            ...
            if auto_join or hub_address:
                if hub_address:
                    ...
                    host, port = hub_address.split(':') if ':' in hub_address else (hub_address, 56700)
                    port = int(port)
                    ...
                    self.hub_discovery = HubDiscovery(transport, key_manager)
                    
                    # Start discovery
                    await self.hub_discovery.start_discovery()
                    
                    # Wait for discovery
                    await asyncio.sleep(5)
                    
                    # Get discovered hubs
                    hubs = self.hub_discovery.get_discovered_hubs()
                    if not hubs:
                    ...
                    hub = hubs[0]
                    logger.info(f"Joining hub {hub.device_id:08X} at {hub.address}:{hub.port}")
                    ...
                    if (current_time - last_data_time) >= interval:
                        if self.protocol.hub_address:
                            ...
                            if sensor_data:
                                ...
                                data_to_send = self._generate_dummy_data(device_type, current_time)
                            
                            # Send data
                            try:
                                await self.protocol.send_sensor_data(data_to_send)
                                logger.info(f"Sent sensor data: {data_to_send}")
                            except Exception as e:
            ...
            if self.hub_discovery:
                await self.hub_discovery.stop_discovery()
    ...
    def _generate_dummy_data(self, device_type: str, timestamp: float) -> Dict[str, Any]:
        ...
        if device_type == 'temperature':
            data['temperature'] = round(20 + random.uniform(-5, 15), 1)  # 15-35°C
            data['unit'] = 'celsius'
        elif device_type == 'humidity':
            data['humidity'] = round(random.uniform(30, 80), 1)  # 30-80%
            data['unit'] = 'percent'
        elif device_type == 'pressure':
            data['pressure'] = round(1013.25 + random.uniform(-50, 50), 2)  # ±50 hPa
            data['unit'] = 'hPa'
        elif device_type == 'light':
            data['illuminance'] = random.randint(0, 1000)  # 0-1000 lux
            data['unit'] = 'lux'
        elif device_type == 'motion':
            data['motion_detected'] = random.choice([True, False])
        elif device_type == 'door':
            data['door_open'] = random.choice([True, False])
        else:
            # Generic sensor
    ...
    def discover_hubs(self, timeout: int = 10):
        ...
        async def _discover():
            ...
            try:
                ...
                key_manager = KeyManager(0x00000000)  # Temporary device ID
...
Examples:
  ...
  shdc-sensor status 0x87654321                    # Show sensor status
    ...
    run_parser.add_argument('device_id', help='Sensor device ID (hex format, e.g., 0x87654321)')
    run_parser.add_argument('device_type', help='Sensor type (temperature, humidity, motion, etc.)')
    run_parser.add_argument('--hub', help='Hub address (host:port, e.g., 192.168.1.100:56700)')
    run_parser.add_argument('--no-auto-join', action='store_true',
    ...
    discover_parser.add_argument('-t', '--timeout', type=int, default=10,
    ...
    if args.command == 'run':
        ...
        try:
            return asyncio.run(cli.run_sensor(
                device_id=device_id,
