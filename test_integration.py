...
class SHDCIntegrationTest:
    ...
    async def start_hub(self, hub_id: int, port: int):
        ...
        self.hub_protocol = SHDCProtocol(
            role=DeviceRole.HUB, device_id=hub_id, port=port
        )
    ...
    async def start_sensor(self, sensor_id: int):
        ...
        # Initialize protocol
        self.sensor_protocol = SHDCProtocol(role=DeviceRole.SENSOR, device_id=sensor_id)
    ...
    async def test_data_transmission(self):
        ...
        for message in self.hub_received_messages:
            if message[0] == "sensor_data":
                ...
                if (
                    device_id == self.sensor_protocol.device_id
                    and data.get("value") == 42.5
                ):
                    data_received = True
                    break
    ...
    async def test_control_messages(self):
        ...
        await self.hub_protocol.send_control_message(
            self.sensor_protocol.device_id, control_data
        )
    ...
    async def test_key_management(self):
        ...
        session_key = hub_key_manager.generate_session_key(
            self.sensor_protocol.device_id
        )
